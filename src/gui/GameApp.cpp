...
Color GetElementColor(unit::Element elem) {
    switch (elem) {
    case unit::Element::Pyro: return RED;
    case unit::Element::Hydro: return BLUE;
    case unit::Element::Anemo: return SKYBLUE;
    case unit::Element::Geo: return GOLD;
    case unit::Element::Electro: return PURPLE;
    case unit::Element::Cryo: return Color{180, 220, 255, 255};
    }
    return WHITE;
}
...
std::string GetElementName(unit::Element elem) {
    switch (elem) {
    case unit::Element::Pyro: return "Pyro";
    case unit::Element::Hydro: return "Hydro";
    case unit::Element::Anemo: return "Anemo";
    case unit::Element::Geo: return "Geo";
    case unit::Element::Electro: return "Electro";
    case unit::Element::Cryo: return "Cryo";
    }
    return "Unknown";
}
...
GameApp::GameApp() {
    ...
    InitWindow(screenWidth, screenHeight, "Synera: Synergy Auto-Arena (3D Raylib)");
    SetTargetFPS(60);
    ...
    // 6 slices gives a hexagon cylinder. Radius 0.47f gives a nice gap with spacing 0.88f.
...
bool GameApp::IsMouseOverUI() {
    ...
    if (m.y >= 500) return true;
    if (m.y <= 60) return true;
    if (m.x >= 20 && m.x <= 240 && m.y >= 120 && m.y <= 480) return true;
    if (m.x >= 1040 && m.x <= 1260 && m.y >= 120 && m.y <= 480) return true;
    if (combat_result_announced_) return true;
    return false;
}
...
void GameApp::Update() {
    ...
    camera_.position = Vector3Lerp(camera_.position, target_cam_pos_, GetFrameTime() * 4.0f);
    camera_.target = Vector3Lerp(camera_.target, target_cam_target_, GetFrameTime() * 4.0f);
    ...
    if (is_combat_ && !combat_result_announced_) {
        ...
        while (combat_tick_timer >= combat_tick_interval) {
            ...
            ProcessCombatTick();
            if (combat_result_announced_) break;
        }
    }
    ...
    for (auto &pair : slimes_) {
        ...
        if (v.move_time < 1.3f) {
            v.move_time += dt * 3.5f;
            if (v.move_time > 1.3f) v.move_time = 1.3f;
        }

        // Attack lunge animation
        if (v.attack_time < 1.0f) {
            v.attack_time += dt * 8.0f;
            if (v.attack_time > 1.0f) v.attack_time = 1.0f;
        }

        // Hurt wobble animation
        if (v.hurt_time < 1.0f) {
            v.hurt_time += dt * 6.0f;
            if (v.hurt_time > 1.0f) v.hurt_time = 1.0f;
        }

        // Cast inflation animation
        if (v.cast_time < 1.0f) {
            v.cast_time += dt * 4.0f;
            if (v.cast_time > 1.0f) v.cast_time = 1.0f;
        }
    }
            ...
            if (auto u_ptr = get_unit(active_board, coord)) {
                ...
                if (slimes_.find(u_ptr) == slimes_.end()) {
                    ...
                    if (stats.owner == unit::Owner::EnemyCtrl) {
                        v.current_pos = { wpos.x, 0.0f, -6.0f }; // Slide/jump in from top
                    } else {
                        v.current_pos = wpos;
                    }
                    v.target_pos = wpos;
                    ...
                    // Attack trigger
                    if (stats.state == unit::State::Attacking && stats.attack_cooldown == stats.attack_interval - 1) {
                        if (v.attack_time >= 1.0f) {
                            v.attack_time = 0.0f;
                            // Find direction to target
                            if (auto target_opt = select_target(active_board, *u_ptr, coord)) {
                                Vector3 target_pos = GetHexWorldPos(*target_opt);
                                v.attack_dir = Vector3Normalize(Vector3Subtract(target_pos, wpos));
                            } else {
                    ...
                    // Cast skill trigger
                    if (stats.mana < v.last_mana && v.last_mana >= stats.max_mana) {
                        v.cast_time = 0.0f;
                        ...
                        if (auto target_opt = select_target(active_board, *u_ptr, coord)) {
                            ...
                            proj.radius = 0.15f;
                            proj.is_area = (unit::element(*u_ptr) == unit::Element::Pyro || unit::element(*u_ptr) == unit::Element::Cryo);
    ...
    for (int i = 0; i < config::engine::BENCH_SIZE; ++i) {
        ...
        if (auto u_ptr = get_unit(active_board, coord)) {
            ...
            if (slimes_.find(u_ptr) == slimes_.end()) {
                VisualSlime v;
                v.current_pos = { wpos.x, 0.0f, 6.0f }; // Slide/jump in from bottom
                v.target_pos = wpos;
                v.move_time = 0.0f; // Slide-in jump
                v.last_coord = coord;
                v.last_hp = stats.hp;
    ...
    for (auto &pair : slimes_) {
        ...
        for (int r = 0; r < config::engine::BOARD_ROWS; ++r) {
            for (int c = 0; c < config::engine::BOARD_COLS; ++c) {
                if (get_unit(active_board, engine::HexCoord{r, c}) == u_ptr) present = true;
            }
        }
        for (int i = 0; i < config::engine::BENCH_SIZE; ++i) {
            if (get_unit(active_board, engine::LinearCoord{i}) == u_ptr) present = true;
        }
    ...
    for (auto it = projectiles_.begin(); it != projectiles_.end(); ) {
        ...
        if (!it->is_area && it->progress < 1.0f) {
            it->current_pos = Vector3Lerp(it->current_pos, it->target_pos, it->progress);
        } else {
            // Expand ring
...
void GameApp::HandleInputs() {
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        if (!over_ui) {
            if (hovered_equip_index_ != -1 && hovered_equip_index_ < (int)session_.equip_pool_.size()) {
                selected_equip_index_ = hovered_equip_index_;
                status_msg_ = "Selected " + GetElementName(static_cast<unit::Element>(hovered_equip_index_ % 6)) + " Drop. Click a unit to equip.";
    ...
    if (is_dragging_ && IsMouseButtonReleased(MOUSE_BUTTON_LEFT)) {
        ...
        if (mouse_pos.x >= 1055 && mouse_pos.x <= 1230 && mouse_pos.y >= 660 && mouse_pos.y <= 705) {
            auto src_u = get_unit(session_.board_, drag_source_);
            if (src_u) {
                auto &stats = unit::stats(*src_u);
                int price = (stats.level == 1) ? 2 : ((stats.level == 2) ? 5 : ((stats.level == 3) ? 14 : 42));
                ...
                status_msg_ = "Sold unit for " + std::to_string(price) + " Gold.";
        ...
        if (!over_ui) {
            ...
            if (drop_found && !(drag_source_ == drop_cell)) {
                ...
                if (std::holds_alternative<engine::HexCoord>(drop_cell)) {
                    ...
                    if (hex.r < 4) {
                        valid = false;
                        status_msg_ = "Cannot place units on Enemy half (rows 0-3)!";
                        status_msg_timer_ = 2.5f;
                    } else {
                        int board_units = 0;
                        for (int r = 4; r < config::engine::BOARD_ROWS; ++r) {
                            for (int c = 0; c < config::engine::BOARD_COLS; ++c) {
                                if (auto u = get_unit(session_.board_, engine::HexCoord{r, c})) {
                        ...
                        bool is_moving_to_empty = !get_unit(session_.board_, drop_cell);
                        bool is_src_bench = std::holds_alternative<engine::LinearCoord>(drag_source_);

                        if (is_src_bench && is_moving_to_empty && board_units >= session_.player_.level) {
                            valid = false;
                            status_msg_ = "Board is full! Upgrade level to deploy more units.";
                            status_msg_timer_ = 2.5f;
                        }
                    }
                }
    ...
    if (selected_equip_index_ != -1 && IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && !over_ui) {
        if (has_hover_) {
            auto u_ptr = get_unit(session_.board_, hovered_coord_);
            if (u_ptr) {
                bool ok = session_.equip_unit(hovered_coord_, selected_equip_index_);
                if (ok) {
...
void GameApp::StartCombatPhase() {
    ...
    if (player_units == 0) {
        status_msg_ = "Deploy at least one unit to the board before starting combat!";
        status_msg_timer_ = 3.0f;
        return;
    }
...
void GameApp::EndCombatPhase() {
    ...
    if (player_won_combat_) {
        ...
        status_msg_ = "VICTORY! Gained " + std::to_string(reward_gold) + " Gold.";
        ...
        if ((rand() % 100) < 30) {
            ...
            unit::Equipment item;
            switch (random_elem) {
            case unit::Element::Pyro: item = unit::PyroDrop{}; break;
            case unit::Element::Hydro: item = unit::HydroDrop{}; break;
            case unit::Element::Anemo: item = unit::AnemoDrop{}; break;
            case unit::Element::Geo: item = unit::GeoDrop{}; break;
            case unit::Element::Electro: item = unit::ElectroDrop{}; break;
            case unit::Element::Cryo: item = unit::CryoDrop{}; break;
            }
            session_.equip_pool_.push_back(item);
            status_msg_ += " Dropped a " + GetElementName(random_elem) + " Drop!";
        }
    } else {
        ...
        for (int r = 0; r < config::engine::BOARD_ROWS; ++r) {
            for (int c = 0; c < config::engine::BOARD_COLS; ++c) {
                if (auto u = get_unit(combat_board_, engine::HexCoord{r, c})) {
                    if (unit::stats(*u).owner == unit::Owner::EnemyCtrl && unit::stats(*u).hp > 0) {
                        survivors++;
                    }
                }
            }
...
void GameApp::DrawGame3D() {
    ...
    for (int r = r_start; r < config::engine::BOARD_ROWS; ++r) {
        for (int c = 0; c < config::engine::BOARD_COLS; ++c) {
            ...
            Color cell_color = (r >= 4) ? Color{50, 110, 80, 255} : Color{110, 50, 50, 255}; // Player area vs Enemy area
            
            // Highlight if selected/hovered
            bool highlight = false;
            if (has_hover_ && std::holds_alternative<engine::HexCoord>(hovered_coord_)) {
                auto hex = std::get<engine::HexCoord>(hovered_coord_);
                if (hex.r == r && hex.c == c) highlight = true;
            }
            if (has_selection_ && std::holds_alternative<engine::HexCoord>(selected_coord_)) {
    ...
    for (int i = 0; i < config::engine::BENCH_SIZE; ++i) {
        Vector3 pos = GetBenchWorldPos(i);
        bool highlight = false;
        if (has_hover_ && std::holds_alternative<engine::LinearCoord>(hovered_coord_)) {
            auto linear = std::get<engine::LinearCoord>(hovered_coord_);
            if (linear.x == i) highlight = true;
        }
        if (has_selection_ && std::holds_alternative<engine::LinearCoord>(selected_coord_)) {
    ...
    for (int i = 0; i < 8; ++i) {
        Vector3 pos = GetEquipWorldPos(i);
        bool eq_highlight = (selected_equip_index_ == i) || (hovered_equip_index_ == i);
        ...
        if (i < (int)session_.equip_pool_.size()) {
            ...
            if (std::holds_alternative<unit::PyroDrop>(eq)) elem = unit::Element::Pyro;
            else if (std::holds_alternative<unit::HydroDrop>(eq)) elem = unit::Element::Hydro;
            else if (std::holds_alternative<unit::AnemoDrop>(eq)) elem = unit::Element::Anemo;
            else if (std::holds_alternative<unit::GeoDrop>(eq)) elem = unit::Element::Geo;
            else if (std::holds_alternative<unit::ElectroDrop>(eq)) elem = unit::Element::Electro;
            else if (std::holds_alternative<unit::CryoDrop>(eq)) elem = unit::Element::Cryo;
    ...
    for (auto &pair : slimes_) {
        ...
        if (v.is_dead && v.death_time >= 1.0f) continue;
        ...
        if (stats.level == 2) base_scale = 0.21f;
        else if (stats.level == 3) base_scale = 0.28f;
        else if (stats.level == 4) base_scale = 0.36f;
        ...
        if (is_dragging_ && drag_source_ == v.last_coord) {
            ...
            if (ray.direction.y < 0) {
                float t = -ray.position.y / ray.direction.y;
                render_pos = Vector3Add(ray.position, Vector3Scale(ray.direction, t));
            }
        ...
        if (v.attack_time < 1.0f) {
            ...
            render_pos = Vector3Add(render_pos, Vector3Scale(v.attack_dir, attack_offset));
        }
        ...
        if (v.cast_time < 1.0f) {
            ...
            visual_scale = Vector3Scale(visual_scale, 1.0f + cast_factor * 0.4f);
        }
        ...
        DrawSlime(render_pos, visual_scale.y, unit::element(*u_ptr), stats.owner, hurt_val, cast_val, is_stunned);

        // Draw small floating sphere for equipped item
        if (!v.is_dead && !std::holds_alternative<std::monostate>(stats.equipped)) {
            Vector3 item_pos = render_pos;
            item_pos.y += visual_scale.y + 0.3f + sin(GetTime() * 4.0f) * 0.06f;
            
            unit::Element elem = unit::Element::Pyro;
            if (std::holds_alternative<unit::PyroDrop>(stats.equipped)) elem = unit::Element::Pyro;
            else if (std::holds_alternative<unit::HydroDrop>(stats.equipped)) elem = unit::Element::Hydro;
            else if (std::holds_alternative<unit::AnemoDrop>(stats.equipped)) elem = unit::Element::Anemo;
            else if (std::holds_alternative<unit::GeoDrop>(stats.equipped)) elem = unit::Element::Geo;
            else if (std::holds_alternative<unit::ElectroDrop>(stats.equipped)) elem = unit::Element::Electro;
            else if (std::holds_alternative<unit::CryoDrop>(stats.equipped)) elem = unit::Element::Cryo;
    ...
    for (const auto &proj : projectiles_) {
        if (!proj.is_area) {
            ...
            DrawSphereWires(proj.current_pos, proj.radius + 0.02f, 8, 8, Fade(proj.color, 0.4f));
        } else {
            // Draw skill blast expansion ring
            DrawCylinderWires(proj.current_pos, proj.radius, proj.radius, 0.05f, 16, Fade(proj.color, 0.8f * (1.0f - proj.radius / proj.max_radius)));
        }
    }
}

void GameApp::DrawHexCell(const Vector3& pos, Color color, bool highlight) {
    // A cylinder with 6 slices is a hexagonal prism. We rotate by 30 degrees (pi/6)
    // around the Y axis so flat edges are parallel to horizontal neighbor lines.
    Vector3 rotationAxis = { 0.0f, 1.0f, 0.0f };
    float rotationAngle = 30.0f; // degrees
    
    DrawModelEx(hex_model_, pos, rotationAxis, rotationAngle, { 1.0f, 1.0f, 1.0f }, color);
    
    // Draw wireframe outline to make cells distinct
    Color wire_color = highlight ? YELLOW : Color{ 35, 35, 45, 255 };
    DrawModelWiresEx(hex_model_, pos, rotationAxis, rotationAngle, { 1.01f, 1.01f, 1.01f }, wire_color);
}

void GameApp::DrawSlime(const Vector3& pos, float scale, unit::Element elem, unit::Owner owner, float hurt_val, float cast_val, bool is_stunned) {
    ...
    if (hurt_val > 0.0f) {
        render_color = ColorAlphaBlend(render_color, RED, Fade(RED, hurt_val * 0.7f));
    } else if (cast_val > 0.0f) {
        render_color = ColorAlphaBlend(render_color, WHITE, Fade(WHITE, cast_val * 0.6f));
    }
    ...
    DrawCubeWires(pos, outer_size, outer_size, outer_size, Fade(base_color, 0.8f));
    
    // Eyes: 2 small black cubes facing forward (negative Z for Player, positive Z for Enemy)
    ...
    if (owner == unit::Owner::PlayerCtrl) {
        Vector3 left_eye = { pos.x - eye_offset_x, eye_height, pos.z - eye_offset_z };
        Vector3 right_eye = { pos.x + eye_offset_x, eye_height, pos.z - eye_offset_z };
    ...
    if (is_stunned) {
        ...
        DrawCylinderWires(stun_pos, scale * 0.7f, scale * 0.7f, 0.02f, 8, YELLOW);
    }
}
...
void GameApp::DrawGame2D() {
    ...
    // Player HP
    DrawText("PLAYER HP:", 30, 20, 20, LIGHTGRAY);
    DrawRectangle(150, 20, 200, 20, DARKGRAY);
    float hp_pct = session_.player_.hp / 100.0f;
    DrawRectangle(150, 20, (int)(200 * hp_pct), 20, hp_pct > 0.4f ? GREEN : RED);
    std::string hp_text = std::to_string(session_.player_.hp) + "/100";
    DrawText(hp_text.c_str(), 220, 22, 16, WHITE);

    // Player Gold
    DrawText("GOLD:", 400, 20, 20, LIGHTGRAY);
    std::string gold_text = std::to_string(session_.player_.gold) + " G";
    DrawText(gold_text.c_str(), 470, 20, 20, GOLD);

    // Player Level
    DrawText("LEVEL:", 580, 20, 20, LIGHTGRAY);
    std::string level_text = std::to_string(session_.player_.level);
    DrawText(level_text.c_str(), 660, 20, 20, SKYBLUE);

    // Board population count (only count PlayerCtrl units)
    int board_units = 0;
    for (int r = 0; r < config::engine::BOARD_ROWS; ++r) {
        for (int c = 0; c < config::engine::BOARD_COLS; ++c) {
            if (auto u = get_unit(is_combat_ ? combat_board_ : session_.board_, engine::HexCoord{r, c})) {
                if (unit::stats(*u).owner == unit::Owner::PlayerCtrl) {
                    board_units++;
                }
            }
    ...
    std::string pop_text = "(" + std::to_string(board_units) + "/" + std::to_string(session_.player_.level) + ")";
    DrawText(pop_text.c_str(), 690, 22, 16, LIGHTGRAY);

    // Round number
    DrawText("ROUND:", 800, 20, 20, LIGHTGRAY);
    std::string round_text = std::to_string(session_.round_);
    DrawText(round_text.c_str(), 890, 20, 20, WHITE);

    // Phase Indicator
    if (is_combat_) {
        DrawRectangle(980, 15, 120, 30, RED);
        DrawText("COMBAT", 1005, 20, 18, WHITE);
    } else {
        DrawRectangle(980, 15, 120, 30, GREEN);
        DrawText("PREPARE", 1000, 20, 18, WHITE);
    }

    // 2. Draw Status Message banner
    if (status_msg_timer_ > 0.0f) {
        int width = MeasureText(status_msg_.c_str(), 20);
        DrawRectangle(640 - width/2 - 20, 80, width + 40, 40, Color{ 20, 20, 25, 220 });
        DrawRectangleLines(640 - width/2 - 20, 80, width + 40, 40, GOLD);
        DrawText(status_msg_.c_str(), 640 - width/2, 90, 20, GOLD);
    }
    ...
    DrawText("ACTIVE RESONANCES", 30, 130, 16, GOLD);
    
    auto active_synergies = unit::compute_synergies(is_combat_ ? combat_board_ : session_.board_);
    
    int sy_y = 170;
    auto draw_synergy_item = [&](const char* name, int count, bool active, Color col) {
        std::string text = std::string(name) + ": " + std::to_string(count);
        DrawText(text.c_str(), 35, sy_y, 14, active ? col : DARKGRAY);
        if (active) DrawRectangle(180, sy_y + 2, 40, 12, col);
        sy_y += 24;
    };

    draw_synergy_item("Pyro (ATK +25%)", active_synergies.pyro_count, active_synergies.fervent_flames, RED);
    draw_synergy_item("Hydro (HP +25%)", active_synergies.hydro_count, active_synergies.soothing_water, BLUE);
    draw_synergy_item("Anemo (Mana -15)", active_synergies.anemo_count, active_synergies.impetuous_winds, SKYBLUE);
    draw_synergy_item("Geo (Shield +15%)", active_synergies.geo_count, active_synergies.enduring_rock, GOLD);
    draw_synergy_item("Electro (Mana+5)", active_synergies.electro_count, active_synergies.high_voltage, PURPLE);
    draw_synergy_item("Cryo (Freeze 20%)", active_synergies.cryo_count, active_synergies.shattering_ice, Color{180, 220, 255, 255});
    draw_synergy_item("Canopy (-15% DMG)", active_synergies.pyro_count + active_synergies.hydro_count + active_synergies.anemo_count + active_synergies.geo_count > 3 ? 4 : 0, active_synergies.protective_canopy, GREEN);
    ...
    DrawText("EQUIPMENT POOL", 1055, 130, 16, GOLD);
    DrawText("(Click to select, then", 1055, 155, 12, GRAY);
    DrawText("click unit to equip)", 1055, 172, 12, GRAY);
    ...
    for (int i = 0; i < (int)session_.equip_pool_.size(); ++i) {
        ...
        if (!is_combat_ && CheckCollisionPointRec(mouse, { (float)x, (float)y, 40, 40 }) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            selected_equip_index_ = i;
            status_msg_ = "Selected " + GetElementName(static_cast<unit::Element>(i % 6)) + " Drop. Click a unit to equip it.";
            status_msg_timer_ = 3.0f;
        }
        ...
        DrawRectangleLines(x, y, 40, 40, selected_equip_index_ == i ? YELLOW : Color{ 50, 50, 65, 255 });
    ...
    DrawText("RECRUITMENT SHOP", 50, 512, 16, GOLD);
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (session_.shop_[i].has_value()) {
            ...
            // Star levels
            std::string stars = "";
            for (int s = 0; s < stats.level; ++s) stars += "*";
            DrawText(stars.c_str(), x + 10, y + 18, 16, YELLOW);

            // Element Name
            std::string name = GetElementName(unit::element(unit)) + " Slime";
            DrawText(name.c_str(), x + 10, y + 42, 14, WHITE);
            
            // Stats summary
            std::string stats_lbl = "HP: " + std::to_string(stats.max_hp) + " ATK: " + std::to_string(stats.atk);
            DrawText(stats_lbl.c_str(), x + 10, y + 68, 10, LIGHTGRAY);

            // Cost tag
            std::string cost_str = "COST: " + std::to_string(cost) + " G";
            DrawText(cost_str.c_str(), x + 10, y + 95, 14, GOLD);
            ...
            bool hover = CheckCollisionPointRec(GetMousePosition(), { (float)btn_x, (float)btn_y, (float)btn_w, (float)btn_h });
            DrawRectangle(btn_x, btn_y, btn_w, btn_h, hover ? Color{ 40, 110, 60, 255 } : Color{ 30, 80, 45, 255 });
            DrawText("BUY", btn_x + btn_w/2 - 14, btn_y + 8, 14, WHITE);
            ...
            DrawText("EMPTY", x + w/2 - 25, y + h/2 - 10, 16, DARKGRAY);
        }
    }

    // Shop actions
    int action_x = 875;
    auto draw_shop_action = [&](const char* text, int y, Color col, Color hover_col, auto action_func) {
        int w = 150;
        int h = 45;
        bool hover = CheckCollisionPointRec(GetMousePosition(), { (float)action_x, (float)y, (float)w, (float)h });
        ...
        DrawText(text, action_x + w/2 - text_w/2, y + 16, 14, WHITE);
    ...
    // Refresh Shop
    draw_shop_action("REFRESH (1G)", 536, Color{ 140, 60, 30, 255 }, Color{ 180, 80, 40, 255 }, [&]() {
    ...
    draw_shop_action(level_btn_lbl.c_str(), 600, Color{ 30, 60, 140, 255 }, Color{ 40, 80, 180, 255 }, [&]() {
        bool ok = session_.buy_level();
        if (!ok) {
            status_msg_ = "Insufficient gold to buy level!";
            status_msg_timer_ = 2.0f;
        } else {
            status_msg_ = "Upgraded player level successfully!";
            status_msg_timer_ = 1.5f;
        }
    });

    // Combat controller buttons on the right side
    auto draw_combat_btn = [&](const char* text, int x, int y, int w, int h, Color col, Color hover_col, auto action_func, bool active) {
        bool hover = active && CheckCollisionPointRec(GetMousePosition(), { (float)x, (float)y, (float)w, (float)h });
        DrawRectangle(x, y, w, h, active ? (hover ? hover_col : col) : DARKGRAY);
        DrawRectangleLines(x, y, w, h, Color{ 60, 60, 70, 255 });
        int text_w = MeasureText(text, 14);
        DrawText(text, x + w/2 - text_w/2, y + h/2 - 7, 14, active ? WHITE : GRAY);
    ...
    draw_combat_btn(combat_lbl.c_str(), 1055, 536, 175, 45, Color{ 30, 110, 45, 255 }, Color{ 40, 140, 55, 255 }, [&]() {
        StartCombatPhase();
    }, !is_combat_);

    // Save Game
    draw_combat_btn("SAVE", 1055, 600, 82, 45, Color{ 70, 70, 75, 255 }, Color{ 90, 90, 95, 255 }, [&]() {
    ...
    // Load Game
    draw_combat_btn("LOAD", 1148, 600, 82, 45, Color{ 70, 70, 75, 255 }, Color{ 90, 90, 95, 255 }, [&]() {
        std::string file = "save_session.txt";
    ...
    // Drag-to-Sell Zone
    if (!is_combat_) {
        bool hover_sell = is_dragging_ && CheckCollisionPointRec(GetMousePosition(), { 1055, 660, 175, 45 });
        DrawRectangle(1055, 660, 175, 45, hover_sell ? Color{ 180, 40, 40, 255 } : Color{ 120, 30, 30, 255 });
        DrawRectangleLines(1055, 660, 175, 45, RED);
        DrawText("DRAG HERE TO SELL", 1055 + 175/2 - MeasureText("DRAG HERE TO SELL", 12)/2, 660 + 16, 12, WHITE);
    }
    ...
    for (auto &pair : slimes_) {
        ...
        if (is_dragging_ && drag_source_ == v.last_coord) {
            ...
            if (ray.direction.y < 0) {
                float t = -ray.position.y / ray.direction.y;
                bar_3d_pos = Vector3Add(ray.position, Vector3Scale(ray.direction, t));
            }
        } else if (v.move_time < 1.0f) {
        ...
        if (screen_pos.x > 0 && screen_pos.x < 1280 && screen_pos.y > 0 && screen_pos.y < 720) {
            int bar_w = 48;
            int bar_h = 4;
            ...
            // Background
            DrawRectangle(x - 1, y - 1, bar_w + 2, bar_h * 2 + 4, Color{ 15, 15, 15, 220 });

            // HP bar
            float hp_ratio = stats.max_hp > 0 ? (float)stats.hp / stats.max_hp : 0.0f;
            Color hp_color = (stats.owner == unit::Owner::PlayerCtrl) ? GREEN : RED;
            DrawRectangle(x, y, (int)(bar_w * hp_ratio), bar_h, hp_color);

            // Shield overlay
            if (stats.shield > 0) {
                float shield_ratio = stats.max_hp > 0 ? (float)stats.shield / stats.max_hp : 0.0f;
                DrawRectangle(x, y, (int)(bar_w * std::min(1.0f, shield_ratio)), 2, Color{ 220, 220, 255, 255 });
            }

            // Mana bar
            float mana_ratio = stats.max_mana > 0 ? (float)stats.mana / stats.max_mana : 0.0f;
            DrawRectangle(x, y + bar_h + 2, (int)(bar_w * mana_ratio), bar_h, BLUE);

            // Level stars
            std::string level_str = "";
            for (int s = 0; s < stats.level; ++s) level_str += "*";
            Color star_col = (stats.level == 4) ? ORANGE : YELLOW;
            DrawText(level_str.c_str(), x + bar_w/2 - MeasureText(level_str.c_str(), 10)/2, y - 11, 10, star_col);
        }
    }
    ...
    if (combat_result_announced_) {
        ...
        DrawRectangleLines(box_x, box_y, box_w, box_h, player_won_combat_ ? GREEN : RED);
        
        std::string title = player_won_combat_ ? "VICTORY!" : "DEFEAT!";
        Color title_color = player_won_combat_ ? GREEN : RED;
        DrawText(title.c_str(), box_x + box_w/2 - MeasureText(title.c_str(), 24)/2, box_y + 30, 24, title_color);
        
        std::string desc = player_won_combat_ ? "You cleared the stage. Obtained Gold!" : "Your forces fell. Lost player health!";
        if (session_.player_.hp <= 0) {
            desc = "Your health reached 0! GAME OVER!";
        }
        DrawText(desc.c_str(), box_x + box_w/2 - MeasureText(desc.c_str(), 14)/2, box_y + 80, 14, LIGHTGRAY);
        ...
        bool hover = CheckCollisionPointRec(GetMousePosition(), { (float)ok_x, (float)ok_y, (float)ok_w, (float)ok_h });
        DrawRectangle(ok_x, ok_y, ok_w, ok_h, hover ? Color{ 50, 50, 60, 255 } : Color{ 40, 40, 50, 255 });
        DrawRectangleLines(ok_x, ok_y, ok_w, ok_h, GRAY);
        DrawText("OK", ok_x + ok_w/2 - MeasureText("OK", 14)/2, ok_y + 10, 14, WHITE);
        ...
        if (hover && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            ...
            if (session_.player_.hp <= 0) {
                ...
                slimes_.clear();
                projectiles_.clear();
                status_msg_ = "New Game Started - Drag and drop units to position them.";
                status_msg_timer_ = 3.0f;
            } else {
                ...
                status_msg_ = "Round " + std::to_string(session_.round_) + " Started! Gained " + std::to_string(base_gold + interest) + " G (Interest: " + std::to_string(interest) + " G).";
